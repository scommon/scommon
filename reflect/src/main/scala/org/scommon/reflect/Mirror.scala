...
/**
 * Reflection utilities for discovering main methods (entry points) and
 * instantiating instances of a type given arguments.
 */
object Mirror {
  ...
  def newInstance[T](description: ClassDescription, classLoader: ClassLoader)(args: Any*): Option[T] =
    newInstanceWithParameterLists[T](description, classLoader)(Seq(args.toSeq))

  def newInstanceWithParameterLists[T](description: ClassDescription, classLoader: ClassLoader)(parameterLists: Iterable[Any]*): Option[T] = {
    val parameterListsAsSeq = (parameterLists map (_.toSeq)).toSeq
    newInstanceWithIterableArgumentTypes[T](description, classLoader,
  ...
  def newInstanceWithArgumentTypes[T](description: ClassDescription, classLoader: ClassLoader)(argTypes: Class[_]*)(args: Any*): Option[T] =
    newInstanceWithIterableArgumentTypes[T](description, classLoader, Seq(argTypes.toSeq), Seq(args.toSeq))

  private[this] def newInstanceWithIterableArgumentTypes[T](description: ClassDescription, classLoader: ClassLoader, argTypes: Seq[Seq[Class[_]]], args: Seq[Seq[Any]]): Option[T] = {
    try {
      val runtime_mirror = universe.runtimeMirror(classLoader)
